...
// Result All
$Sql_All = "SELECT * FROM complaints_suggestion WHERE Cs_To = 'Manager' ORDER BY CS_Date DESC, CS_Id DESC";
$Result_All = mysqli_query($link,$Sql_All);
$num_All = mysqli_num_rows($Result_All);

// Result Today
$Sql_Today = "SELECT * FROM complaints_suggestion WHERE Cs_To = 'Manager' And CS_Date = '$Date_Tody'   ORDER BY CS_Date DESC, CS_Id DESC";
$Result_Today = mysqli_query($link,$Sql_Today);
$num_Tody = mysqli_num_rows($Result_Today);


// Result Old
$Sql_Old = "SELECT * FROM complaints_suggestion WHERE Cs_To = 'Manager' And CS_Date < '$Date_Tody'   ORDER BY CS_Date DESC, CS_Id DESC";
...
while($row = mysqli_fetch_array($Result_Today)){ ?>
    ...
    echo " لا توجدأي طلبات أو شكاوي جديدة";
...
while($row = mysqli_fetch_array($Result_Old)){ ?>
      ...
      <h4> رقم الشكوى / الاقتراح : <?php echo $row['CS_Id']; ?> #</h4>
        ...
        echo " لا توجدأي طلبات أو شكاوي سابقة";
    }
}
